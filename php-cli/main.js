...
export default async function phpCli(
  ...
  const caBundlePath = path.join(dirname, 'ca-bundle.crt')
    //new URL('ca-bundle.crt', (import.meta || {}).url).pathname;
  ...
  args.unshift('-d', `openssl.cafile=${caBundlePath}`);
	...
	const defaultPhpIniPath = path.join(dirname, 'php.ini')
    // await import('./php.ini');
  ...
  //   const phpWasmCommand =
  //     `${process.argv[0]} ${process.execArgv.join(
  //       ' '
  //     )} ${process.argv[1]}`;
